...
import type { AdminServer, AdminServerFilters } from '../../types/admin';
...
export default function AdminServers() {
  ...
  const [showDeleteModal, setShowDeleteModal] = useState<AdminServer | null>(null);
  ...
  useEffect(() => {
    const filters: AdminServerFilters = {};
    if (searchDebounce) filters.search = searchDebounce;
    if (onlineFilter !== 'all') filters.online = onlineFilter === 'online';
    fetchServers(0, serversPageSize, filters);
  }, [searchDebounce, onlineFilter, serversPageSize]);
  ...
  const handlePageChange = (newPage: number) => {
    ...
    if (searchDebounce) filters.search = searchDebounce;
    if (onlineFilter !== 'all') filters.online = onlineFilter === 'online';
    fetchServers(newPage, serversPageSize, filters);
  };

  const handleDisconnect = async (server: AdminServer) => {
  ...
  const handleDelete = async () => {
    ...
    setActionLoading(true);
    try {
      await deleteServer(showDeleteModal.id);
  ...
  const totalPages = Math.ceil(serversTotal / serversPageSize);
      ...
      {serverStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
            <p className="text-2xs font-mono uppercase tracking-wider text-slate-500 mb-1">Total Servers</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">{serverStats.totalServers}</p>
          </div>
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
            <p className="text-2xs font-mono uppercase tracking-wider text-slate-500 mb-1">Online</p>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{serverStats.onlineServers}</p>
          </div>
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
            <p className="text-2xs font-mono uppercase tracking-wider text-slate-500 mb-1">Offline</p>
            <p className="text-2xl font-bold text-slate-500">{serverStats.offlineServers}</p>
          </div>
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
            <p className="text-2xs font-mono uppercase tracking-wider text-slate-500 mb-1">New Today</p>
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{serverStats.newServersToday}</p>
          </div>
        </div>
      )}
        ...
        {loadingServers && servers.length === 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 font-mono text-sm uppercase tracking-wider">
              {searchDebounce || onlineFilter !== 'all' ? 'No servers found matching your filters' : 'No servers found'}
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                    ...
                    <th className="px-4 py-3 text-left text-2xs font-mono uppercase tracking-wider text-slate-500">Connections</th>
                    <th className="px-4 py-3 text-left text-2xs font-mono uppercase tracking-wider text-slate-500">Collaborators</th>
                    <th className="px-4 py-3 text-left text-2xs font-mono uppercase tracking-wider text-slate-500">Last Seen</th>
                  ...
                  {servers.map((server) => (
                    <tr key={server.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        {server.groupName && (
                          <p className="text-xs text-slate-500 mt-0.5">{server.groupName}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm text-slate-600 dark:text-slate-400">{server.ownerEmail}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={clsx(
                          'inline-flex items-center gap-1.5 text-xs font-medium',
                          server.online ? 'text-green-600 dark:text-green-400' : 'text-slate-500'
                        )}>
                          <span className={clsx(
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {server.online && (
                            <button
                              ...
                              className="p-1.5 rounded-lg text-amber-500 hover:text-amber-600 hover:bg-amber-500/10 transition-colors disabled:opacity-50"
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-800">
                <div className="text-sm text-slate-500">
                  Showing {serversPage * serversPageSize + 1} to {Math.min((serversPage + 1) * serversPageSize, serversTotal)} of {serversTotal}
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handlePageChange(serversPage - 1)}
                    disabled={serversPage === 0 || loadingServers}
                  ...
                  <span className="text-sm text-slate-500">
                    Page {serversPage + 1} of {totalPages}
                  </span>
                  <button
                    onClick={() => handlePageChange(serversPage + 1)}
                    disabled={serversPage >= totalPages - 1 || loadingServers}
                    className="px-3 py-1.5 text-sm font-medium rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    Next
                  </button>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl overflow-hidden">
              ...
              <div className="p-6">
                ...
                <p className="text-slate-600 dark:text-slate-400 mb-6">
                  Are you sure you want to delete <span className="font-medium text-slate-900 dark:text-white">{showDeleteModal.name}</span>?
